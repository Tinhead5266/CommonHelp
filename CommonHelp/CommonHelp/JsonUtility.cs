...
namespace CommonHelp.CommonHelp
{
    public static class JsonUtility
    {
        ...
        public static string ToDate(this DateTime dtime)
        {
            return dtime.Year + "-" + (dtime.Month + 1) + "-" + dtime.Day;
        }
        /// <summary>
        /// 返回对象序列化 特定类型 特定格式
        /// </summary>
            ...
            foreach (var fieldInfo in objectType.GetFields())
            {
                ...
                if (fieldInfo.FieldType.IsValueType)
                {
                    ...
                    else if (fieldInfo.FieldType == typeof(DateTime) || fieldInfo.FieldType == typeof(DateTime?))
                    {
                        if (fieldValue != null)
                        {
                            result.Add(fieldInfo.Name, string.Format("{0:yyyy-MM-dd HH:mm:ss}", fieldValue));
                        }
                        else
                        {
